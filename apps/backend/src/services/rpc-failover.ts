...
class RPCFailoverService {
  ...
  private initializeProvider(provider: RPCProvider): void {
    try {
      ...
      logger.info(`✓ Initialized RPC provider: ${provider.name}`);
    } catch (error) {
  ...
  private selectBestProvider(): RPCProvider | null {
    ...
    if (healthyProviders.length === 0) {
      logger.error('No healthy RPC providers available, attempting recovery...');
  ...
  async executeWithFailover<T>(
    ...
    for (const provider of this.providers) {
      ...
      try {
        ...
        logger.info(`✓ ${operationName} succeeded with ${provider.name}`);
        return result;
      } catch (error: any) {
        lastError = error;
        logger.warn(
