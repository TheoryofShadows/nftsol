...
class _MockPool {
  ...
  async connect() {
    this.connected = true;
    logger.info('[Mock DB] Connected to mock database');
    
    return {
      ...this,
      release: () => {
        this.connected = false;
        logger.info('[Mock DB] Connection released');
      },
      on: (event: string, callback: (...args: any[]) => void) => {
        logger.info(`[Mock DB] Event: ${event}`, callback);
        return this;
      },
      once: (event: string, callback: (...args: any[]) => void) => {
        logger.info(`[Mock DB] Event (once): ${event}`, callback);
        return this;
      },
      removeListener: () => this,
  ...
  async end() {
    this.connected = false;
    logger.info('[Mock DB] Connection pool ended');
    return Promise.resolve();
  }

  // Event emitter methods
  on(event: string, _listener: (...args: any[]) => void) {
    logger.info(`[Mock DB] Added listener for ${event}`);
    return this;
  }

  once(event: string, _listener: (...args: any[]) => void) {
    logger.info(`[Mock DB] Added one-time listener for ${event}`);
    return this;
  }

  removeListener(event: string, _listener: (...args: any[]) => void) {
    logger.info(`[Mock DB] Removed listener for ${event}`);
    return this;
  }

  removeAllListeners(event?: string | symbol) {
    logger.info(`[Mock DB] Removed all listeners${event ? ` for ${event.toString()}` : ''}`);
    return this;
  }
}
...
async function createPoolWithRetry(attempt = 1): Promise<Pool> {
  try {
    ...
    logger.info('✅ Database pool created (will test on first query)');
    return pool;
  } catch (error) {
    ...
    logger.warn(`⚠️  Database connection attempt ${attempt} failed, retrying in ${RETRY_DELAY}ms...`);
...
async function getPool(): Promise<DatabasePool> {
  ...
  if (poolInstance && poolInstance._pool) {
    ...
    pgPool.on('error', (err: Error, client: PoolClient) => {
      logger.error('⚠️  Unexpected error on idle database client:', err.message);
      // Gracefully handle the error without crashing
      if (client) {
        try {
          client.release(err);
        } catch (releaseErr) {
          logger.error('Error releasing client:', releaseErr);
        }
      }
    });
    ...
    const healthCheckInterval = setInterval(async () => {
      try {
        ...
        if (health.status !== 'healthy') {
          logger.warn('⚠️  Database health check failed:', health.details.message);
        }
      } catch (error) {
        logger.warn('Health check error:', error instanceof Error ? error.message : error);
      }
...
const initializeDatabase = async () => {
  ...
  try {
    if (process.env.NODE_ENV === 'development') {
      ...
      logger.info('✅ Development database will connect on first query');
    } else {
      // In production, just test the connection
      logger.info('🔹 Testing production database connection...');
      const health = await pool.healthCheck();
      logger.info(`✅ Database connection ${health.status}: ${health.details.message}`);
    }
  } catch (error) {
    logger.error('⚠️  Database initialization warning (will retry on first query):', (error instanceof Error ? error.message : error));
...
const dbInitTimeout = setTimeout(() => {
  logger.warn('⚠️  Database initialization timeout - proceeding without DB connection');
}, 3000);

initializeDatabase().catch(err => {
  logger.warn('⚠️  Database initialization failed, will retry on first query:', err instanceof Error ? err.message : err);
}).finally(() => {
...
process.on('uncaughtException', async (error) => {
  ...
  try {
    ...
    logger.error('Error closing database connections during uncaught exception:', e);
  }
  process.exit(1);
});
