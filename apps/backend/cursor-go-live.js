...
const main = async () => {
  ...
  try {
    ...
    console.log("📊 Solana Health:", health.data.data.solana.healthy ? "CONNECTED" : "DISCONNECTED");
    ...
    try {
      const mint = await retryRequest(() =>
        axios.post(`${BASE_URL}/api/nfts/mint`, mintPayload)
      );
        ...
        try {
          ...
          try {
            ...
            if (process.data.success) {
              ...
              console.log("🔗 Explorer:", `https://explorer.solana.com/tx/${process.data.data.transactionSignature}?cluster=devnet`);
            } else {
