...
export async function fetchFromGateways(path: string) {
  let lastErr: any;
  for (const g of gateways) {
    ...
    for (let r = 0; r <= RETRIES; r++) {
      try {
        ...
        const res = await axios.get(url, { responseType: "arraybuffer", timeout: TIMEOUT, headers:{Accept:"*/*"} });
        if (res.status >= 200 && res.status < 300) return { buf: Buffer.from(res.data), headers: res.headers };
      } catch (e) { lastErr = e; await new Promise(s => setTimeout(s, BACKOFF * (r + 1))); }
    }
  }
  throw lastErr;
}
